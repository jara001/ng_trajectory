...
help: ## Show this help message.
	...
	@grep "##@[^ \"]*" $(MAKEFILE_LIST) | grep --color=auto -F -v grep | sed -e "s/^.*##@\\([a-zA-Z][a-zA-Z]*\\).*\$$/\1/" | sed "/^\\$$/d" | sort | uniq | xargs -n 1 -I'{}' bash -c "echo; echo {} targets:; grep '##@{}' Makefile | sed -e 's/##@{}//' | column -c2 -t -s :"
