...
P = ParameterList()
add_parameter!(P, Parameter("hold_map", false, false, Bool, "When true, the map is created only once.", "init"))
add_parameter!(P, Parameter("range_limit", 0, 0, float, "Maximum distance from the center of the segment. 0 disables this.", ""))
add_parameter!(P, Parameter("reserve_width", false, false, Bool, "When true, the segments are reserved a path towards both walls.", ""))
add_parameter!(P, Parameter("reserve_selected", [], [], Array, "IDs of segments that should use the reservation method, when empty, use all.", ""))
...
function segmentate(points, group_centers; overflown...)
    ...
    update_all!(P, overflown, reset = false)

    _groups = [ [] for _ in 1:length(group_centers)]

    if get_value(P, "reserve_width") == false
        _map = copy(MAP)
        _map[_map .== 100] .= 255
        ...
        for (_i, _c) in enumerate(eachrow(points_to_map(group_centers)))
            ...
            if length(get_value(P, "reserve_selected")) > 0 && occursin(get_value(P, "reserve_selected"), _i)
                continue
            end
            ...
            for (_vi, (_vx, _vy)) in enumerate(zip(getindex.(valids, 1), getindex.(valids, 2)))
                ...
                if _distance < get_value(P, "reserve_distance")
                    _map[_vx, _vy] = 100 + _i
                end
                if _vi % 1000 == 0
        ...
        for _a in -1:1
            for _b in -1:1
                ...
                if _cell == 255 || (_cell == 100 + _map[cell[1], cell[2]])
                    ...
                    queue = [queue; [cell[1] + _a, cell[2] + _b]']
                    # push!(queue, [cell[1] + _a, cell[2] + _b])
                end
            end
        end
    ...
    if get_value(P, "range_limit") <= 0
        return [g for g in groups if length(g) > 0]
    else
        return [x[sqrt.(sum.(( x[:, 1:2] .- group_centers[ix][:2])^2, dims = 2)) < get_value(P, "range_limit")] for (ix, x) in enumerate(groups) if length(x) > 0]
    end
end
