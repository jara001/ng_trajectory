...
def determine_opponent_side(traj_pos_now, traj_pos_prev, opponent_pos):
    """Determine a side of the opponent relative to ego trajectory.

    Arguments:
    traj_pos_now -- current trajectory point, 1D ndarray
    traj_pos_prev -- previous trajectory point, 1D ndarray
    opponent_pos -- opponent position, 1D ndarray

    Returns:
    side -- float
        1.0 === opponent left, 0.0 === opponent mid, -1.0 === opponent right
    """
    ...
    return numpy.sign(
        ego_vector[0] * opponent_vector[1] - ego_vector[1] * opponent_vector[0]
    )


def do_polygons_intersect(a: numpy.array, b: numpy.array) -> bool:
    """Determine whether there is an intersection between two polygons.

    The two polygons described are by the lists of vertices.
    Uses the Separating Axis Theorem.

    Arguments:
    a -- an ndarray of connected points [[x_1, y_1], [x_2, y_2],...]
         that form a closed polygon
    b -- an ndarray of connected points [[x_1, y_1], [x_2, y_2],...]
         that form a closed polygon

    Returns:
    True if there is any intersection between the 2 polygons, otherwise False
    ...
    for i in range(len(polygons)):
        ...
        for i1 in range(len(polygon)):
            ...
            # looking at separates the two polygons,
...
def get_rect_points(
    center: numpy.ndarray, dims: Tuple[float, float], angle: float
) -> numpy.array:
    """Obtain four corners of the rectangle.

    Bottom left is the first corner, from there it goes counter clockwise.

    Arguments:
    center -- center point of the rectangle, (2,) numpy.ndarray
    dims -- dimensions (length, breadth) of the rectangle, tuple of floats
    angle -- rotation of the rectangle, rad, float

    Returns:
    corners -- corners of the rectangle, (4, 2) numpy.ndarray
    """
