...
def trajectoryClosest(point: numpy.ndarray, ipoints: numpy.ndarray) -> int:
    ...
    indices = numpy.where(
        (ipoints[:, 0] == point[0]) & (ipoints[:, 1] == point[1])
    )[0]
...
def trajectoryPerpendicular(
        point: numpy.ndarray,
        ipoints: numpy.ndarray,
        dipoints: numpy.ndarray) -> Callable[[float], float]:
    """Find a perpendicular line at specified point on the interpolated path.

    Arguments:
    point -- point where the perpendicular line should be situated,
             1x2 numpy.ndarray
    ipoints -- interpolated path, nx(>=2) numpy.ndarray
    dipoints -- first order derviation of the interpolated path
             nx(>=2) numpy.ndarray
...
def lineEndpointBorderObtain(
    ...
    while abs(step) > (endpoint_accuracy * dfactor):
        point = numpy.asarray([[
            center[0] + dist + step, line(center[0] + dist + step)
        ]])

        if not numpy.any(
            numpy.all(numpy.abs(numpy.subtract(_valid_points, point)) < [
                math.sqrt(2) * endpoint_accuracy,
                math.sqrt(2) * endpoint_accuracy
            ], axis = 1)
        ):
            step /= 2
...
def create(
        ...
        endpoint_distance: float,
        endpoint_accuracy: float,
        line_reduction: float) -> List[numpy.ndarray]:
    ...
    endpoint_distance -- starting distance from the center, float
    endpoint_accuracy -- accuracy of the center-endpoint distance, float
    line_reduction -- factor by which the number of line points is lowered
                      before internal interpolation, float
    ...
    for point in group_centers:
        ...
        point1 = lineEndpointBorderObtain(
            p,
            point,
            endpoint_distance,
            endpoint_accuracy,
        ...
        point2 = lineEndpointBorderObtain(
            p,
            point,
            endpoint_distance,
            endpoint_accuracy,
...
def transform(
        ...
        cuts: List[Tuple[numpy.ndarray]]) -> numpy.ndarray:
    """Transform points from 1D transformed to real coordinates.

    Arguments:
    points -- points to transform (from transformed coordinates
              to real coordinates), nx1 numpy.ndarray
